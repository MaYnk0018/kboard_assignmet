...
function LoginContent() {
  ...
  const emailFormik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        setLoading(true);
        let res = await AxiosService.post(`${ApiRoutes.AUTH.path}`, values);
        if (res.status === 200) {
        ...
        {mobileLoginClicked ? (
          !isOtpGenerated ? (
            <Form
              ...
              className="authForm mx-auto p-5 rounded-5"
            ...
            >
              ...
              <div className="d-grid mb-2">
                <Button className="formBtns" type="submit" disabled={loading}>
                  {loading ? <Spinner animation="border" /> : "Send OTP"}
                </Button>
              </div>
              <hr style={{ color: "blue" }} />
            ...
            >
              <Form.Group className="mb-4">
                ...
                <Form.Control
                  ...
                  id="mobile"
                  name="mobile"
                  value={sharedMobileNumber}
                  readOnly
                />
                {otpFormik.touched.mobile && otpFormik.errors.mobile && (
                  <div className="authErrorText">{otpFormik.errors.mobile}</div>
                )}
              </Form.Group>
              ...
              <Form.Group className="mb-4">
                ...
                {otpFormik.touched.otp && otpFormik.errors.otp && (
                  <div className="authErrorText">{otpFormik.errors.otp}</div>
                )}
              </Form.Group>

              <div className="d-grid mb-2">
                <Button className="formBtns" type="submit" disabled={loading}>
                  {loading ? <Spinner animation="border" /> : "Verify OTP"}
                </Button>
              </div>
              <hr style={{ color: "blue" }} />
              <div className="text-center mt-3">
          ...
          >
            <Form.Group className="mb-4">
              ...
              {emailFormik.touched.email && emailFormik.errors.email && (
                <div className="authErrorText">{emailFormik.errors.email}</div>
              )}
            </Form.Group>
            ...
            <div className="text-center mt-3">
              New User?{" "}
              <Link href={"register"} className="loginText">
                Register
              </Link>
            </div>
            <hr style={{ color: "blue" }} />
