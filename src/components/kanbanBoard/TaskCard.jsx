...
const TaskCard = ({ task, index }) => {
  const taskNewTitle = useRef(null)
  const taskNewDescription = useRef(null)
  const taskNewStatus = useRef(null)
  const [show, setShow] = useState(false)
  const [loading, setLoading] = useState(false)
  const [taskData, setTaskData] = useState([])
  const formattedDateTime = useFormattedDateTime(task.updatedAt)
  ...
  const handleEditTask = async (taskId) => {
    ...
    try {
      const res = await AxiosService.put(`${ApiRoutes.TASK.path}/${taskId}`, updatedDatas, {
  ...
  const handleDeleteTask = async (taskId) => {
    try {
      const res = await AxiosService.delete(`${ApiRoutes.TASK.path}/${taskId}`, {
        headers: { 'Authorization': `${getLoginToken}` }
      })
      if (res.status === 200) {
        handleClose()
  ...
  const getTaskData = async (taskId) => {
    try {
      const res = await AxiosService.get(`${ApiRoutes.TASK.path}/${taskId}`, {
        headers: { 'Authorization': `${getLoginToken}` }
      })
            ...
            <div className='taskCardContainer d-flex'>
              <div className='d-flex mt-2' style={{ width: "100%" }}>
                ...
                <div className='buttons d-flex'>
                  <div className='editBtn' onClick={() => handleEditShow(task.taskId)}>
                    <FontAwesomeIcon icon={faEdit} />
                  </div>
                  <div className='deleteBtn' onClick={() => handleDeleteTask(task.taskId)}>
      ...
      <Modal show={show} onHide={handleClose}>
        <Form>
          ...
          <Modal.Body>
            <Form.Group className="mb-3">
              ...
              <Form.Control
                ...
                defaultValue={taskData[0]?.taskDetails[0]?.taskTitle}
                type="text"
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control
                defaultValue={taskData[0]?.taskDetails[0]?.taskDescription}
                type="text"
                placeholder="Enter here"
                name='taskDescription'
                ref={taskNewDescription}
              />
            </Form.Group>
            <Form.Group className="mb-3">
              ...
              <Form.Select
                ref={taskNewStatus}
                defaultValue={taskData[0]?.taskDetails[0]?.taskStatus}
              >
          ...
          <Modal.Footer>
            ...
            <Button
              variant="primary"
              onClick={() => handleEditTask(taskData[0]?.taskId)}
              disabled={loading}
            >
